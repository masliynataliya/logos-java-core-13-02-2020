...
public class Deputy extends Person implements Comparable<Deputy> {
    ...
    public void bribeDeputy() {
        ...
        if (grafter) {
            if (amount >= 5000) {
                System.out.println("Міліція увязнить цього депутата" + getName() + " " + getSurname());
            } else {
                bribe = amount;
            }
    ...
    public int compareTo(Deputy deputy) {
        return Integer.compare(getBribe(), deputy.getBribe());
        //        if (getBribe() > deputy.getBribe()) {
//            return 1;
//        }else if(getBribe() < deputy.getBribe()){
//            return -1;
//        }else return 0;
//    }
    }
